...
{
	final int array_of_choice[] = new int[100];
	double price = 0.00;
	int i = 0;
	int cost;
	int doagain;
...
public double delete(final int i, double price, final int array_of_choice[]) {
	System.out.println("do you want to delete anything from your order");
	System.out.println("yes=1,no=0");
	int j;
	final int delete_choice = sc.nextInt();
	if (delete_choice == 1) {
		for (j = 0; j < i; j++) {
		...
		final int d_quantity = sc.nextInt();
